...
function squad() {
  return [
    { no: 1, name: 'A1', g: 'M', team: 'A', avg: 150, delta: 5, strikes: 3, spares: 2 },
    { no: 2, name: 'A2', g: 'M', team: 'A', avg: 120, delta: 2, strikes: 1, spares: 4 },
    { no: 3, name: 'A3', g: 'F', team: 'A', avg: 110, delta: null, strikes: 0, spares: 1 },
    { no: 4, name: 'B1', g: 'M', team: 'B', avg: 140, delta: 8, strikes: 4, spares: 3 },
    { no: 5, name: 'B2', g: 'M', team: 'B', avg: 130, delta: -1, strikes: 2, spares: 2 },
    { no: 6, name: 'B3', g: 'F', team: 'B', avg: 100, delta: 3, strikes: 1, spares: 0 },
    { no: 7, name: 'C1', g: 'M', team: 'C', avg: 90, delta: null, strikes: 0, spares: 0 },
    { no: 8, name: 'C2', g: 'F', team: 'C', avg: null, delta: null, strikes: 0, spares: 0 },
    { no: 9, name: 'U1', g: 'M', team: null, avg: 200, delta: 9, strikes: 5, spares: 5 },
  ];
}
...
test('teamStandings sums series, strikes, spares and female score per team', () => {
  ...
  assert.equal(st.C.series, 90);        // C2 has no average
  assert.equal(st.C.avg, 90);           // only one averaged player
  assert.equal(st.C.female, 0);         // C2 has no average
});

test('compareTeams orders by series then the official tie-breakers', () => {
  const ranked = compareTeams(squad());
  assert.deepEqual(ranked.map(r => r.team), ['A', 'B', 'C']); // 380 > 370 > 90
